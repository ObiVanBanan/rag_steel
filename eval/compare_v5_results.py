"""Compare the latest V5 evaluation outputs."""
...
def _validate_payloads(
    ...
    if missing:
        raise ValueError(
            "Cannot compare V5 runs: missing artifacts for "
            + ", ".join(sorted(missing))
        )
    ...
    if len(set(cases_by_name.values())) > 1:
        raise ValueError(
            "Cannot compare V5 runs: mismatched case counts "
            + json.dumps(cases_by_name, ensure_ascii=False)
        )
    if len(set(sha_by_name.values())) > 1:
        raise ValueError(
            "Cannot compare V5 runs: mismatched dataset sha256 "
...
def compare_v5_results(
    *,
    ...
    lines = [
        ...
        "## Key Metrics",
        (
            f"- DeepSeek brand interpretation accuracy: "
            f"`{_metric(deepseek, 'brand_interpretation_accuracy'):.4f}`"
        ),
        (
            f"- DeepSeek article identity accuracy: "
            f"`{_metric(deepseek, 'article_identity_accuracy'):.4f}`"
        ),
        (
            f"- DeepSeek hard exact match: "
            f"`{_metric(deepseek, 'hard_interpretation_exact_match'):.4f}`"
        ),
        (
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Compare V5 evaluation outputs.")
