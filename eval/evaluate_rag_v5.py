"""Evaluate the V5 search pipeline with gold extraction inputs."""
...
from eval.v5_constants import CATEGORY_ORDER, DEFAULT_GOLDEN_DATASET_PATH, DEFAULT_RAG_RESULTS_PATH
from eval.v5_schema import EvalCase, ExpectedAttributes
from rag_steel.attribute_extractor import QueryAttributes
...
def _requested_from_response(response: Any) -> ExpectedAttributes:
    ...
    if hasattr(raw_requested, "model_dump"):
        ...
        requested_dump = dict(raw_requested)
    brand = requested_dump.get("brand")
    ...
    for field in (
        ...
        if field in {"dn", "pn_bar"}:
            try:
                ...
                wrong_fields.append(field)
        elif field in {"brand", "resolved_brand"}:
            ...
            if _article_comparison_key(expected_value) != _article_comparison_key(actual_value):
                wrong_fields.append(field)
...
def _evaluate_case(
    ...
    engine.attribute_extractor = GoldV5Extractor(case.expected_attributes)
...
def evaluate_rag_v5(
    *,
    ...
    engine = engine_factory(
        embedder=embedder,
        attribute_extractor=GoldV5Extractor(ExpectedAttributes()),
    )
...
def render_report(payload: dict[str, Any], output_path: Path) -> str:
    ...
    lines.extend(["", "## Eligible Hit@5 Failures", ""])
    ...
    lines.extend(["", "## By Resolution Mode"])
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate RAG for V5.")
    ...
    parser.add_argument("--output-md", type=Path, default=Path("eval/rag_v5_report.md"))
    return parser


def main(argv: list[str] | None = None) -> int:
    args = _build_arg_parser().parse_args(argv)
    payload = evaluate_rag_v5(
