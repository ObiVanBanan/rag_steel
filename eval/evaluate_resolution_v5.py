"""Evaluate the deterministic V5 resolution layer."""
...
from eval.v5_constants import DEFAULT_GOLDEN_DATASET_PATH, DEFAULT_RESOLUTION_RESULTS_PATH
...
def _compare_expected_actual(
    ...
    for field in ("brand", "resolved_brand", "article", "resolved_article"):
...
def _evaluate_case(
    ...
    resolution = engine.query_resolver.resolve(
        brand=case.expected_attributes.brand,
        article=case.expected_attributes.article,
    ...
    actual = ExpectedAttributes(
        brand=resolution.brand.raw if resolution.brand else None,
    ...
    if (
        ...
        and case.expected_attributes.brand is not None
...
def _build_diagnostics(cases: list[ResolutionCaseResult]) -> dict[str, Any]:
    ...
    return {
        "false_correction_cases": [
            {
                ...
                "category": case.category,
                "query": case.query,
                "expected_raw_brand": case.expected.get("brand"),
                "expected_resolved_brand": case.expected.get("resolved_brand"),
                "actual_raw_brand": case.actual.get("brand"),
            ...
            for case in false_correction_cases
        ],
        ...
        "missing_resolved_brand_cases": [
            ...
            for case in missing_resolved_brand_cases
        ],
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate resolution for V5.")
    ...
    parser.add_argument("--output-md", type=Path, default=Path("eval/resolution_v5_report.md"))
    return parser


def main(argv: list[str] | None = None) -> int:
    args = _build_arg_parser().parse_args(argv)
    payload = evaluate_resolution_v5(dataset_path=args.dataset, max_cases=args.max_cases)
