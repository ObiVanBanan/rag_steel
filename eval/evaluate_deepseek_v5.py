"""Evaluate DeepSeek extraction for V5."""
...
from eval.v5_constants import DEFAULT_DEEPSEEK_RESULTS_PATH, DEFAULT_GOLDEN_DATASET_PATH
...
def _hard_exact_match_rate(cases: list[DeepSeekCaseResult]) -> float:
    if not cases:
        return 0.0
    correct = sum(
        1
        for case in cases
        if not case.comparison["wrong_fields"]
...
def _technical_error_rate(cases: list[DeepSeekCaseResult]) -> float:
    if not cases:
        return 0.0
    technical_failures = sum(
        1 for case in cases if case.error_type is not None or case.invalid_response
    )
    return technical_failures / len(cases)


def _semantic_correction_success_rate(cases: list[DeepSeekCaseResult]) -> float:
    semantic_cases = [case for case in cases if case.category in _SEMANTIC_CORRECTION_CATEGORIES]
    ...
    successful = sum(
        ...
        and not case.comparison["hallucinated_fields"]
        and not case.comparison["missing_fields"]
        and case.error_type is None
    )
    return successful / len(semantic_cases)


def _ambiguous_case_safety_rate(cases: list[DeepSeekCaseResult]) -> float:
    safety_cases = [case for case in cases if case.category in _AMBIGUOUS_SAFETY_CATEGORIES]
    if not safety_cases:
        return 0.0
    safe = sum(
        1
    ...
    return safe / len(safety_cases)


def evaluate_deepseek_v5(
    *,
    ...
    summary = {
        "cases": len(cases),
        "brand_interpretation_accuracy": _field_accuracy(cases, "brand"),
        "article_identity_accuracy": _field_accuracy(cases, "article"),
        "dn_interpretation_accuracy": _field_accuracy(cases, "dn"),
    ...
    payload = {
        ...
        "by_category": {
            category: {
                "cases": len(items),
                "brand_interpretation_accuracy": _field_accuracy(items, "brand"),
                "hard_interpretation_exact_match": _hard_exact_match_rate(items),
            }
...
def render_report(payload: dict[str, Any], output_path: Path) -> str:
    ...
    lines = [
        ...
        f"- brand interpretation accuracy: `{summary['brand_interpretation_accuracy']:.4f}`",
        f"- article identity accuracy: `{summary['article_identity_accuracy']:.4f}`",
        (
            f"- dn / pn / connection accuracy: "
            f"`{summary['dn_interpretation_accuracy']:.4f}` / "
            f"`{summary['pn_interpretation_accuracy']:.4f}` / "
            f"`{summary['connection_interpretation_accuracy']:.4f}`"
        ),
        f"- hard interpretation exact match: `{summary['hard_interpretation_exact_match']:.4f}`",
        f"- hallucination rate: `{summary['hallucination_rate']:.4f}`",
        f"- technical error rate: `{summary['technical_error_rate']:.4f}`",
        f"- semantic correction success rate: `{summary['semantic_correction_success_rate']:.4f}`",
        f"- ambiguous-case safety rate: `{summary['ambiguous_case_safety_rate']:.4f}`",
        (
            f"- latency p50 / p95: "
    ...
    for failure in payload["failures"][:20]:
        lines.append(
            f"- `{failure['id']}` `{failure['query']}` wrong={failure['wrong_fields']} "
            f"missing={failure['missing_fields']} hallucinated={failure['hallucinated_fields']} "
...
def _build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Evaluate DeepSeek extraction for V5.")
    ...
    parser.add_argument("--output-md", type=Path, default=Path("eval/deepseek_v5_report.md"))
    return parser


def main(argv: list[str] | None = None) -> int:
    args = _build_arg_parser().parse_args(argv)
    payload = evaluate_deepseek_v5(dataset_path=args.dataset, max_cases=args.max_cases)
